...
install_docker() {
  ...
  run_command "Installing Docker packages" "sudo apt-get update && sudo apt-get install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin docker-compose"
...
run_command "Installing fstail" "sudo wget -q -O /usr/local/bin/fstail https://github.com/alexellis/fstail/releases/download/0.1.0/fstail && sudo chmod +x /usr/local/bin/fstail"
